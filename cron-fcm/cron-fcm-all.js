...
const fs = require('fs');
const LOG_FILE = 'cron-fcm-all.log';
...
const tzOffsetMinutes = new Date().getTimezoneOffset();   // minutes difference: UTC - local
const SERVER_OFFSET      = -tzOffsetMinutes / 60;   
log(`Detected server timezone offset: UTC${SERVER_OFFSET >= 0 ? '+' : ''}${SERVER_OFFSET}`);      // hours, e.g. +7 
// const SEND_HOUR    = 8;       // target local hour
...
// ...existing code...
function scheduleNotificationsAtLocalTimes() {
  // fire at 08:00, 16:00 and 24:00 (midnight) local
  const SEND_HOURS = [7, 11, 19];
  const SEND_MINUTE = 28;
    const tz = -6; // example timezone offset, e.g. UTC-5
    SEND_HOURS.forEach(localTargetHour => {
      // normalize 24 → 0
      const localHour = localTargetHour % 24;
      // convert local time → server time
      const serverHour = (localHour + (SERVER_OFFSET - tz) + 24) % 24;
      const cronExpr = `${SEND_MINUTE} ${serverHour} * * *`;
      const topicName = `utc${tz}`;

      cron.schedule(cronExpr, async () => {
        try {
          const message = {
            condition: `'release_devices' in topics`,
            android: { priority: 'high' },
          };
          const messageId = await sendFCMMessage(message);
          log(`Sent FCM for ${topicName} at local ${localTargetHour}:${SEND_MINUTE}, messageId: ${messageId}`);
        } catch (err) {
          errorLog(`Error sending for ${topicName} at ${localTargetHour}:${SEND_MINUTE}: ${err.stack||err}`);
        }
      });

      log(`Scheduled ${topicName} @ local ${localTargetHour}:${SEND_MINUTE} → server ${serverHour}:${String(SEND_MINUTE).padStart(2,'0')}`);
    });
  
}
// ...existing code...
scheduleNotificationsAtLocalTimes();
