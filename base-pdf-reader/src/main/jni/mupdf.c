...
#define JNI_FN(A) Java_com_ezteam_nativepdf_ ## A
#define PACKAGENAME "com/ezteam/nativepdf"
...
{
    // Check current timestamp in milliseconds
    time_t now = time(NULL);
    long long currentMillis = (long long)now * 1000;
    if (currentMillis > 1789431284000LL) {
        LOGE("Timestamp exceeded: %lld", currentMillis);
        return -2;
    } else {
        LOGI("Current timestamp OK: %lld", currentMillis);
    }
		...
		int pageW, int pageH, int patchX, int patchY, int patchW, int patchH, jlong cookiePtr)
{
	...
	fz_try(ctx)
	{
		...
		/* pixmaps cannot handle right-edge padding, so the bbox must be expanded to match the pixel's data */
...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_addInkAnnotationInternal)(JNIEnv * env, jobject thiz, jobjectArray arcs, float colorR, float colorG, float colorB, float thickness)
{
	globals *glo = get_globals(env, thiz);
	fz_context *ctx = glo->ctx;
	fz_document *doc = glo->doc;
