package free.pdf.documents.pdfreader.pdfviewer.pdfeditor
...
import free.pdf.documents.pdfreader.pdfviewer.pdfeditor.di.appModule
...
import free.pdf.documents.pdfreader.pdfviewer.pdfeditor.screen.start.SplashActivity
...
//                )
    ...
    private fun initLanguage() {
        if (TextUtils.isEmpty(PreferencesHelper.getString(PreferencesHelper.KEY_LANGUAGE))) {
            free.pdf.documents.pdfreader.pdfviewer.pdfeditor.screen.language.LocaleManager.getInstance(this).prefLanguage =
                free.pdf.documents.pdfreader.pdfviewer.pdfeditor.screen.language.LocaleManager.LANGUAGE_DEFAULT
            val currentLanguage = resources.configuration.locales.get(0).language
            for (i in free.pdf.documents.pdfreader.pdfviewer.pdfeditor.screen.language.LocaleManager.lstCodeLanguage.indices) {
                if (currentLanguage == free.pdf.documents.pdfreader.pdfviewer.pdfeditor.screen.language.LocaleManager.lstCodeLanguage[i]) {
